...
public class LocalAIAgent {
    ...
    private String buildRankPrompt(String candidateSnapshotText, String marketCtx, String historyCtx) {
        ...
        sb.append("\n【硬性要求】只能依据上面列出的具体数字和状态做判断，不得引用大盘走势、宏观经济等" +
                ...
                "如果某支股票的状态说明里写着「持仓中」，说明用户已经买入，理由必须围绕加仓/减仓/继续" +
                "持有展开；如果同时写着「T+1前不可卖」，今天完全不可能卖出，理由里不能出现任何暗示" +
                "可以卖出的表述。只有状态说明里写着「未持仓，仍是候选观察股」的股票，理由才可以讨论" +
                "要不要买入、什么价位买入——绝不能把这套买入话术用在已经持仓的股票上，这是最容易出的" +
                "错误，务必仔细核对每支股票的持仓状态之后再落笔。\n");
        sb.append("\n【输出要求】按打分从高到低排列，每支股票三行，不要写其他内容：\n");
        sb.append("股票：名称(代码)\n评分：0-100整数\n理由：不超70字，必须引用至少一个上面给出的具体数字，且符合上面的持仓状态要求\n");
        return sb.toString();
    }
